...
const UPDATE_LOG = [
  ...
  },
  {
    ...
    title: '灵宠与藏宝阁界面整理',
    items: [
      '灵宠独立界面改为更清楚的轻量卡片布局：优先展示等级最高灵宠、出战灵宠、喂养道具、升星与放生操作',
      '藏宝阁抽取界面整理为保底、八折次数、抽取按钮分区显示，只优化界面，不改概率与数值',
    ],
  },
  {
    version: 'v57',
  ...
  },
  {
    ...
    title: '存档安全强化',
    items: [
      '自动存档升级为多版本备份：每次保存自动保留最近 5 个不同版本，主档异常时可回退到更早的存档，不再只剩一份备份',
      '存档恢复页新增「历史版本」区：可查看最近 5 个自动存档版本并按需一键恢复',
    ],
  },
  {
    version: 'v54',
    ...
    title: '灵宠榜参评优化',
    items: [
      '灵宠榜改用等级最高的灵宠参评（不再只看当前出战灵宠），等级相同比星级，养得最强的那只替你上榜',
    ],
  },
  {
    version: 'v53',
    ...
    title: '战斗面板显示法术暴击',
    items: [
      '修复法术暴击不显示：战斗面板原只显示物理暴击率，叠法爆装备后数值纹丝不动，现改为「物暴 X% · 法暴 Y%」分开显示',
    ],
  },
  {
    version: 'v52',
    ...
    title: '灵宠一键升星 · 星级统计',
    items: [
      '灵宠界面新增星级统计：一眼看清当前 1星/2星/3星…各有多少只灵宠',
      '新增「⚡ 一键升星」：自动把同名同星达 3 只的灵宠逐组升星，支持连锁升星（9 只 1 星自动合成 1 只 3 星），无需逐只数着点',
      '一键升星前会弹出确认，显示将升次数与灵石消耗；每组保留培养最优的主宠（出战宠绝不消耗），其余两只作素材',
    ],
  },
  {
    version: 'v51',
    date: '2026-08-21',
    title: '名望榜 · 灵宠榜接入全球天机榜',
    items: [
      '名望榜、灵宠榜接入天机榜全球数据：不再只是本地假名排名，而是与境界/战力/秘境三榜一样，实时同步全球道友成绩',
      '名望榜按名望值全球论道；灵宠榜按出战灵宠的星级、等级全球论道，未出战灵宠的道友不参与灵宠榜',
      '游戏内斗法台的名望/灵宠本地榜保持不变，天机榜内查看全球排名',
      '离线成绩照常暂存，联网后自动同步上榜',
    ],
  },
  {
  ...
  },
  {
    ...
    title: '洞府与灵宠修复 · 排行榜补全',
    items: [
      '修复洞府灵田数据：16种作物、种子、六类肥料、资源作物与30级洞府现已完整生效；旧洞府存档会自动迁移，已种植作物保留成长进度',
      '灵宠升星优化：始终保留参与合成中等级最高的灵宠及其技能、神品天赋、好感和培养数据；最高等级相同时可自行选择主宠',
      '斗法台排行榜补回名望榜与灵宠榜：名望榜可累计解锁「声名鹊起／威震四海／名扬天下／声名震寰宇」称号；灵宠榜按出战宠物的星级、等级排序',
      '策划测试存档不再参与本地排行榜或天机榜；此前已上榜的测试档会在启用后退出天机榜',
      '本次同时更新资源版本与缓存，联网玩家会收到更新提示；确认刷新前会自动保存当前存档',
    ],
  },
  {
    version: 'v48',
...
// 名望榜为本地模拟排行；保留百席，使各档名望称号均可正常判定。
const FAME_LADDER = [
  { name: '大罗剑尊', fame: 15000 }, { name: '丹道圣手', fame: 12500 }, { name: '万妖之主', fame: 10800 },
  { name: '九霄真人', fame: 9300 }, { name: '玄冥老祖', fame: 8100 }, { name: '赤霄仙子', fame: 7000 },
  { name: '太虚散人', fame: 6100 }, { name: '沧澜剑仙', fame: 5300 }, { name: '青木尊者', fame: 4600 }, { name: '紫电真君', fame: 4000 },
  ...Array.from({ length: 90 }, (_, i) => ({ name: `云游修士·${String(i + 11).padStart(3, '0')}`, fame: Math.max(50, 3800 - i * 42) })),
];

// 灵宠榜以星级优先、等级次之；仅出战灵宠参与排名。
const PET_LADDER = [
  { name: '剑痴·慕容', petId: 'shenlong', star: 8, level: 120 }, { name: '丹霞仙子', petId: 'fenghuang', star: 7, level: 130 },
  { name: '血手人屠', petId: 'tuntunshu', star: 7, level: 110 }, { name: '青莲剑客', petId: 'qinglong', star: 6, level: 100 },
  { name: '白眉老道', petId: 'huofeng', star: 6, level: 90 }, { name: '黑风双煞', petId: 'baihu', star: 5, level: 85 },
  { name: '黄雀道人', petId: 'jinpeng', star: 5, level: 75 }, { name: '快剑阿七', petId: 'huoya', star: 4, level: 60 },
  { name: '铁掌水上漂', petId: 'xuanwu', star: 3, level: 45 }, { name: '无名散修', petId: 'linglu', star: 2, level: 30 },
];
...
const STORY_NODES = {
  ...
  arena_ladder: {
    ...
    choices: [
      { label: '返回排行榜', next: 'rank_hall' },
    ],
  },

  rank_hall: {
    title: '排行榜',
    text: '榜上留名，方知仙途谁主沉浮。请选择要查看的榜单。',
    choices: [
      { label: '⚔ 斗法天梯', next: 'arena_ladder' },
      { label: '📯 名望榜', next: 'fame_ladder' },
      { label: '🐾 灵宠榜', next: 'pet_ladder' },
      { label: '返回斗法台', next: 'arena_hall' },
    ],
  },
  ...
  fame_ladder: {
    title: '名望榜',
    dynamicText: (s) => getFameLadderText(s),
    choices: [{ label: '返回排行榜', next: 'rank_hall' }],
  },

  pet_ladder: {
    title: '灵宠榜',
    dynamicText: (s) => getPetLadderText(s),
    choices: [{ label: '返回排行榜', next: 'rank_hall' }],
  },
...
const REDEEM_CODES = {
  ...
  // 策划测试码：当前存档不再参与任何排行榜。
  '90756238': { excludeFromRanking: true },
  '80571346': { items: [
